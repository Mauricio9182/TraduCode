...
def reset_all():
    global tree_nodes, tree_edges, translation_parts, symbol_table
...
def do_translate():
    ...
    while True:
        ...
        symbol_table.append({
            ...
            'traduccion': (
                tok.value
                if tok.type == 'PUNTUACION'
                else translate_word(tok.value) or '???'
            ),
            'linea': tok.lineno
        })
    ...
    # SEMÁNTICA
    # =========================================
    semantic_errors = validate_semantics(symbol_table)
    ...
    return jsonify({
        ...
        'success': (
            len(syntax_errors) == 0
            and len(error_table) == 0
            and len(semantic_errors) == 0
        )
    })
